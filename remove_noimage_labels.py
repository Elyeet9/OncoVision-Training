...
for folder in os.listdir(labels_dir):
    for file in os.listdir(os.path.join(labels_dir, folder)):
        # check if labels_dir/folder/file.txt exists for images_dir/folder/file.png
        if not os.path.exists(os.path.join(images_dir, folder, file.replace('.txt', '.png'))):
            # remove images_dir/folder/file.png
            os.remove(os.path.join(labels_dir, folder, file))
            count_removed += 1
